...
#
# The system is otherwise a single hue all the way down: the surfaces are
# green-tinted stone (#F5F8F6 has G above R and B, so there is no true neutral
# on the ramp at all), the alternating tint is viridian, the aurora is viridian,
# the accents and the footer accent are viridian. A palette with one hue has
# nothing to measure itself against, and reads as a cast rather than a choice.
#
# Two steps, because it has exactly one job — the availability status dot — and
# a scale would invite it to spread. 62 degrees is an ochre: far enough from
# viridian's 152 to register as a different family, and far enough from
# `danger` (29) not to read as an error. It sits close to `warning` (70) in hue
# and is separated by value and chroma instead — 1.56:1 apart, which at these
# sizes is the difference between an amber dot and a brown one.
#
# L 0.620 is not a taste value: it is the lightest step that still clears
# WCAG 1.4.11's 3:1 against the page as a non-text indicator (3.24:1). Anything
# brighter is a prettier amber and an illegible dot.
WARM_HUE = 62.0
WARM = {"300": (0.780, 0.130), "600": (0.620, 0.150)}
...
def main() -> None:
    ...
    # Step 0 is pure white, and it is not an oversight that it has no chroma.
    # Every other value in the system carries a hint of the brand hue — which
    # is what makes it a family, and also what leaves the eye nowhere truly
    # neutral to rest. surfaceRaised is white so that a raised panel reads as
    # a clean sheet laid on a tinted page rather than a slightly brighter patch
    # of the same tint, and so light mode has a genuine elevation step at all
    # (surface and surfaceRaised used to be the same stone-50: 1.0:1).
    stone = {"0": "#FFFFFF", **build_scale(NEUTRAL_HUE, NEUTRAL_CHROMA)}
    ...
    tokens["color"]["warm"] = {
        "$comment": (
            "Generated by build_palette.py. The system's ONE warm counterpoint, at hue 62 "
            "against viridian's 152. Two steps only, because it has exactly one job: the "
            "availability status dot, light and dark. 600 at L 0.620 is the lightest step "
            "that still clears WCAG 1.4.11's 3:1 against the page (3.24:1) as a non-text "
            "indicator. Do not scatter this — a second warm accent and the counterpoint "
            "stops being one."
        ),
        **annotate({k: oklch_to_hex(L, C, WARM_HUE) for k, (L, C) in WARM.items()}),
    }
    tokens["color"]["stone"] = {
        "$comment": (
            ...
            "0 is pure white and carries no chroma at all: it is the raised-panel surface "
            "in light mode, and the only true neutral in the system."
        ),
        **annotate(stone),
    }
