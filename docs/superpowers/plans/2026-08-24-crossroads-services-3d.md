...
- **Eager JavaScript must not grow.** three.js is reached through a plain dynamic `import()` inside the mount effect and must never enter First Load JS. Not `next/dynamic`, which wraps React components: the scene is a plain module. This constraint said `next/dynamic` until Task 6 found it was the wrong tool.
...
const eagerPaths = [...new Set([...html.matchAll(/src="([^"]+\.js)"/g)].map((m) => m[1]))].map(toPath);
